...
### Key Features:
- **Absolute Numbers**: ADR is expressed in absolute terms, so its value must be adapted to the asset being traded. For example,&nbsp;**BTC**&nbsp;may have an ADR of&nbsp;**1000**, while a low-value meme coin may have an ADR of only **0.1**.- **Daily Focus**: The ADR is specifically designed to measure daily volatility. Other periods (e.g., weekly or monthly) are not supported.
...
- **Set ADR Thresholds**: Configure the bot to enter long trades when the ADR drops below a specific threshold (e.g., 2000 for BTC) and short trades when the ADR rises above a high threshold (e.g., 3000 for BTC).- **Automate Entries and Exits**: The bot will automatically enter and exit trades based on ADR thresholds that indicate shifts in daily volatility.
...
#### Implementation in Gainium
- **Set ADR-Based Targets**: Use the ADR to set realistic daily price targets and stop-losses. For example, if the ADR for an asset is 1000, set targets at or near this level for more predictable movements.- **Automate Price Targets**: The bot can automatically close trades when price targets (based on the ADR) are met, ensuring you capitalize on the asset’s daily range.
